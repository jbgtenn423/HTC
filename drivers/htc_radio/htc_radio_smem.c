...
static int htc_radio_smem_probe(struct platform_device *pdev)
{
	...
	// pr_err("[smem]%s: SMEM_ID_VENDOR0:%d\n", __func__, SMEM_ID_VENDOR0);

	ret = qcom_smem_alloc(SMEM_APPS, SMEM_ID_VENDOR0, sizeof(struct htc_smem_type));

	if (ret < 0 && ret != -EEXIST) {
		pr_err("%s: unable to allocate SMEM_APPS shared state entry\n", __func__);
		return ret;
	}

	htc_radio_smem_via_smd = qcom_smem_get(SMEM_APPS, SMEM_ID_VENDOR0, &size);

	//addr = qcom_smem_virt_to_phys(htc_radio_smem_via_smd);
	//pr_err("[smem]%s: SMEM_APPS address:%lx\n", __func__, addr);

	if (IS_ERR(htc_radio_smem_via_smd)) {
		pr_err("%s: Unable to acquire vendor0 item\n", __func__);
		return PTR_ERR(htc_radio_smem_via_smd);
	}

	// pr_err("[smem]%s: size:%d\n", __func__, size);
