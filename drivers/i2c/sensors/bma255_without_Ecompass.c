...
#define	ASENSE_TARGET_BMA255E			0x00B4		/*180*/
#define	ASENSE_TARGET_BMA250E			0x02D0		/*720*/
	...
	const char *buf, size_t count)
{
    ...
    error = kstrtoul(buf, 10, &enable);
    if (error)
	return error;
    D("bma250_enable_interrupt, power_key_pressed = %d\n", power_key_pressed);
    if(enable == 1 && !power_key_pressed){ // Slope interrupt

	cir_flag = 1;

	/*Set the related parameters*/
	error = bma250_set_Int_Mode(this_client, 1);/*latch interrupt 250ms*/

	error += bma250_set_slope_duration(this_client, 0x01);//dur+1
	error += bma250_set_slope_threshold(this_client, 0x07);//0x07 * 3.91  =

	/*Enable the interrupts*/
	error += bma250_set_Int_Enable(this_client, 5, 1);//Slope X
	error += bma250_set_Int_Enable(this_client, 6, 1);//Slope Y
	error += bma250_set_Int_Enable(this_client, 7, 0);//Slope Z
	...
	if (error)
	    return error;
	D("Always Ready enable = 1 \n");

    }  else if(enable == 0){

	error += bma250_set_Int_Enable(this_client, 5, 0);//Slope X
	error += bma250_set_Int_Enable(this_client, 6, 0);//Slope Y
	error += bma250_set_Int_Enable(this_client, 7, 0);//Slope Z
	...
	const char *buf, size_t count)
{
    ...
    int error;
    error = kstrtoul(buf, 10, &powerkey_pressed);
    if (error)
	return error;
...
static DEVICE_ACCESSORY_ATTR(enable_cir_interrupt, S_IWUSR|S_IWGRP,
...
static int BMA_pinctrl_init(void)
{
        ...
        gdata->gpio_state_init = pinctrl_lookup_state(gdata->pinctrl, "bma255_acc_intr_init");
...
static int bma250_parse_dt(struct device *dev, struct bma250_platform_data *pdata)
{
	...
	pdata->power_LPM = NULL;/*bma250_sr_lpm;*/
		...
		const struct i2c_device_id *id)
{
	...
	if (!bma->input_cir) {
	    kfree(bma);
	    input_free_device(akm->input_dev);//free the successful dev and return
	    return -ENOMEM;
	}
...
#endif
	atomic_set(&a_flag, 1);
	D("%s: BMA250_NO_COMP: I2C retry 10 times version. OK\n", __func__);
	I("%s: BMA250_NO_COMP: Reduce log. OK\n", __func__);
