...
/* Etherent interface wan port define */
#define SUPPORT_WAN_PORT 1
...
#if SUPPORT_WAN_PORT
	int hcd_core_id;
	struct workqueue_struct *eth0_work_queue;
	struct delayed_work	eth0_delay_work;
#endif
};
...
#define ETH0_DELAY_TIME 7000 /* in ms */

static int rtl8152_carrier = 0;
static bool eth0_reset = false;

/* To get carrier status.
 */
int get_carrier(void)
{
	return rtl8152_carrier;
}
EXPORT_SYMBOL(get_carrier);

void set_reset_flag(bool reset) {
	eth0_reset = reset;
}
EXPORT_SYMBOL(set_reset_flag);

/* To get the core id info from  root hub of xhci.
 */
static
int get_roothub_core_id(struct usb_interface *intf)
{
...
/* Do not suspend in 7s after probing
 */
static void eth0_delay_suspend_work(struct work_struct *work) {
	struct r8152 *tp = container_of(work, struct r8152, eth0_delay_work.work);
	if (rtl8152_carrier == 0) {
		usb_enable_autosuspend(tp->udev);
		pr_info("%s r8152: enable autosuspend\n",__func__);
	}
}
#endif
			 ...
			 const struct usb_device_id *id)
{
...
#if SUPPORT_WAN_PORT
	core_id = get_roothub_core_id(intf);
	if (core_id) {
		netdev = alloc_netdev_mqs(sizeof(struct r8152), "eth0", NET_NAME_UNKNOWN, ether_setup, 1, 1);
	} else {
		netdev = alloc_netdev_mqs(sizeof(struct r8152), "eth_rtl%d", NET_NAME_UNKNOWN, ether_setup, 1, 1);
	}
#else
	...
	if (tp->hcd_core_id) {
		rtl8152_carrier = 0;
		INIT_DELAYED_WORK(&tp->eth0_delay_work, eth0_delay_suspend_work);
		tp->eth0_work_queue = create_singlethread_workqueue("eth0_wq");
		if (!tp->eth0_work_queue) {
			pr_err("%s: Unable to create workqueue eth0_wq\n", __func__);
			destroy_workqueue(tp->eth0_work_queue);
			goto out1;
		}
		if (eth0_reset) {
			queue_delayed_work(tp->eth0_work_queue, &tp->eth0_delay_work, msecs_to_jiffies(ETH0_DELAY_TIME));
			netif_info(tp, probe, netdev, "eth0_reset_flag = %d\n", eth0_reset);
			eth0_reset = false;
		} else
			usb_enable_autosuspend(udev);
	}
	netif_info(tp, probe, netdev, "HCD core id = %d\n", tp->hcd_core_id);
#else
	//usb_enable_autosuspend(udev);
#endif
...
#if SUPPORT_WAN_PORT
		if (tp->hcd_core_id) {
			pr_info("%s: Destroy workqueue eth0_wq\n", __func__);
			destroy_workqueue(tp->eth0_work_queue);
		}
#endif
