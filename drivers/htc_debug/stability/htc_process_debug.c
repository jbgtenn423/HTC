 ...
 */
	...
	if (t->comm[0]) {
		...
		list_for_each_entry(tc, &task_comm_list, list) {
			if (sig != SIGCHLD && tc->comm[0] && (!strncmp(t->comm, tc->comm, TASK_COMM_LEN)) ) {
...
void do_group_exit_debug_dump(int exit_code)
{
	...
	if (t->comm[0]) {
		read_lock(&task_comm_lock);
		list_for_each_entry(tc, &task_comm_list, list) {
			if (tc->comm[0] && (!strncmp(t->comm, tc->comm, TASK_COMM_LEN)) ) {
...
static int __init track_process_init(void)
{
	...
	if (!process_kobj)
		...
		ret = sysfs_create_group(process_kobj, &attr_group);
		#if 0 /* default add system_server, zygote, zygote64 to task_comm_list */
		if(0 == ret){
			task_comm_list_store(NULL, NULL, "system_server\0", 14);
			task_comm_list_store(NULL, NULL, "main\0", 5);
		}
		#endif
	}
