...
ssize_t bldr_last_log_read_once(char __user *userbuf, size_t klog_size)
{
	ssize_t len = 0;
	...
	if (0 < len) {
		if (copy_to_user(userbuf, (const void*)bl_last_log_buf, len)) {
...
ssize_t bldr_log_read_once(char __user *userbuf, size_t klog_size)
{
	ssize_t len = 0;
	...
	if (0 < len) {
		if(copy_to_user(userbuf, (const void*)bl_cur_log_buf, len)) {
