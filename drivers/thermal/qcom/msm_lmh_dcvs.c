...
void htc_lmh_stat_show(void)
{
	...
	int printed_cpu_num[] = {0, 4, 7};  // only print CPU0 ,CPU4 and CPU7

        for (i = 0; i < ARRAY_SIZE(printed_cpu_num); i++) {
                memset(output, 0, sizeof(output));
                for (j = CPU_FREQ_SPACING_NUM-1; j >= 0; j--){
			memset(piece, 0, sizeof(piece));
			snprintf(cpu_num_str, sizeof(cpu_num_str),"%s%d:", "CPU", printed_cpu_num[i]);
			snprintf(piece, sizeof(piece), "%s%d(%d)",
				strlen(output)>0 ? "," : cpu_num_str,
				j,
				htc_lmh_stat[printed_cpu_num[i]][j].count);
