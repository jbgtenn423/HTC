...
{
	uint32_t cpu_id = cpu%MAX_CORES_PER_CLUSTER;
	uint32_t cluster_id = cpu/MAX_CORES_PER_CLUSTER;

	return (cluster_id * MAX_CORES_PER_CLUSTER + cpu_id);
}
...
#if 0
/* MSM8998 PSCI done in TZ. no more need. */
