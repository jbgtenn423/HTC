...
#ifdef HTC_FEATURE
	char image_config_id[CONFIG_ID_STRING_SIZE] = {0};
#endif
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
...
#ifdef HTC_FEATURE
		snprintf(image_config_id + 2 * ii, 3, "%02X", fwu->img.ui_config.data[ii]);

		if (fwu->img.ui_config.data[ii] != fwu->config_id[ii]) {
			flash_area = UI_CONFIG;
			for (ii++; ii < config_id_size; ii++)
				snprintf(image_config_id + 2 * ii, 3, "%02X", fwu->img.ui_config.data[ii]);
			goto exit_config;
		}
#else
...
static void get_panel_id(struct synaptics_rmi4_data *rmi4_data)
{
	int temp_panel_ID;
	char *panel_ID;
	int s_len = strlen("htc_panel_id=0x");
	panel_ID = strstr(panel_str, "htc_panel_id=0x");
	if (panel_ID != NULL) {
		strlcpy(panel_ID, panel_ID + s_len, 9);
		temp_panel_ID = (int)simple_strtoull(panel_ID, NULL, 16);
		rmi4_data->panel_ID = (temp_panel_ID >> 8) & 0xFF;
	} else {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: cannot get htc_panel_id",
				__func__);
	}
}
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
#ifdef HTC_FEATURE
		get_panel_id(rmi4_data);
		for (i = 0; i < bdata->panel_id_num; i++) {
			if (bdata->panel_id_array[i] == rmi4_data->panel_ID) {
				retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
						bdata->touch_image_name[i],
						strlen(bdata->touch_image_name[i]),
						strlen(bdata->touch_image_name[i]));
				fwu->image_name[MAX_IMAGE_NAME_LEN - 1] = '\0';
				break;
			}
		}
		if (retval < 0 || fwu->image_name == NULL) {
#else
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
#elif defined(HTC_FEATURE)
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
#endif
	rmi4_data = NULL;
//#ifdef HTC_FEATURE
//	if (!strcmp(htc_get_bootmode(), "download")
//		|| !strcmp(htc_get_bootmode(), "RUU")) {
//		dev_dbg(rmi4_data->pdev->dev.parent,
//				"%s: skip touch fw update in download/ruu mode\n",
//				__func__);
//		return;
//	}
//#endif
...
#ifdef HTC_FEATURE
static int __init get_panel_str(char *str)
{
	panel_str = str;
	pr_info(" %s: panel_str: %s\n", __func__, panel_str);
	return 0;
} early_param("msm_drm.dsi_display0", get_panel_str);
#endif
