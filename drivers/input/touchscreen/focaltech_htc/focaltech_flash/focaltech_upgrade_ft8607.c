...
*****************************************************************************/
u8 pb_file_ft8607[] = {
#include "../include/pramboot/FT8607_Pramboot_V0.6_20170526.img"
};
...
static int read_3gamma(struct i2c_client *client, u8 **gamma, u16 *len)
{
    ...
    if (gamma_len > MAX_GAMMA_LEN) {
        FTS_ERROR("gamma data len(%d) is too long just ignore", gamma_len);
#if 0
        return -EINVAL;
#endif
    }
...
static int fts_ft8607_lic_upgrade(struct i2c_client *client, u8 *buf, u32 len)
{
    ...
    if (ret < 0) {
        FTS_ERROR("initial code invalid, but still upgrade lcd init code");
#if 0
        return -EINVAL;
#endif
    }
