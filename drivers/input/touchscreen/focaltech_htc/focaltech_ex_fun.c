...
static ssize_t fts_fwupgradebin_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    fts_upgrade_bin(client, fwname, 0, isHtc);
...
static ssize_t fts_fwforceupg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    fts_upgrade_bin(client, fwname, 1, isHtc);
...
#if HTC_FEATURE
static ssize_t fts_debug_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct fts_ts_data *ts_data = fts_data;
    return snprintf(buf, PAGE_SIZE, "%08X\n", ts_data->debug_mask);
}

static ssize_t fts_debug_store(struct device *dev, struct device_attribute *attr,  const char *buf, size_t count)
{
    struct fts_ts_data *ts_data = fts_data;

    if (sscanf(buf, "%ux", &ts_data->debug_mask) != 1) {
        return -EINVAL;
    }
...
#if HTC_FEATURE
static DEVICE_ATTR(debug_level, (S_IWUSR|S_IRUGO), fts_debug_show, fts_debug_store);
#endif
