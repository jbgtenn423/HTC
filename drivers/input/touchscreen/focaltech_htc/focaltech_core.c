...
static unsigned int tamper_flag = 0; // to check whether device is s-on on not
#endif
/*****************************************************************************
...
static int fts_input_report_b(struct fts_ts_data *data)
{
...
#ifdef CONFIG_TOUCHSCREEN_ROTATE_FTS_90_DEGREE
            input_report_abs(data->input_dev, ABS_MT_POSITION_X, events[i].y);
            input_report_abs(data->input_dev, ABS_MT_POSITION_Y, data->pdata->x_max - events[i].x);
#else
...
#if !HTC_FEATURE
#if CONFIG_TOUCHSCREEN_ROTATE_FTS_90_DEGREE
            FTS_DEBUG("[B]P%d(%d, %d)[p:%d,tm:%d] DOWN!", events[i].id, events[i].y,
                      data->pdata->x_max - events[i].x, events[i].p, events[i].area);
#else
    ...
    for (i = 0; i < data->touch_point; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            if(data->debug_mask & TOUCH_DOWN_UP_LOG)
                events[i].status = 1;
        } else if (EVENT_UP(events[i].flag)) {
            if(data->debug_mask)
                FTS_DEBUG("[B]P%d UP!", events[i].id);
            events[i].status = 0;
        }
...
#endif
#ifdef CONFIG_TOUCHSCREEN_ROTATE_FTS_90_DEGREE
    input_set_abs_params(input_dev, ABS_MT_POSITION_X, pdata->y_min, pdata->y_max, 0, 0);
    input_set_abs_params(input_dev, ABS_MT_POSITION_Y, pdata->x_min, pdata->x_max, 0, 0);
#else
...
#ifdef CONFIG_TOUCHSCREEN_FTS_USE_DSI_NOTIFY
static void htc_dsi_status_detect(int status)
{
    struct fts_ts_data *ts_data = fts_data;

    FTS_INFO("DSI notifier, %d", status);
...
/*****************************************************************************
*  Name: drm_notifier_callback
*  Brief: the notifier be used since sdm845,
*         and flow is similar to FB but return in not primary display
*  Input:
*  Output:
*  Return:
*****************************************************************************/
                                 ...
                                 unsigned long event, void *data)
{
    ...
    struct fts_ts_data *fts_data =
        container_of(self, struct fts_ts_data, drm_notifier);

    if (evdata && evdata->data &&
        fts_data && fts_data->client) {
        transition = evdata->data;
#if HTC_FEATURE
        if(fts_data->debug_mask)
#endif
           FTS_DEBUG("DRM blank %d, event %ld, id %d", *transition, event, evdata->id);
        ...
        if (event == MSM_DRM_EVENT_BLANK) {
            ...
            } else if (*transition == MSM_DRM_BLANK_POWERDOWN) {
                fts_ts_suspend(&fts_data->client->dev);
            }
        } else if (event == MSM_DRM_EVENT_PANEL_RESET_TOGGLE) {
            if (evdata->id != MSM_DRM_PRIMARY_DISPLAY)
                return 0;
            if (*transition == MSM_DRM_RESET_HIGH) {
                usleep_range(5, 5);
                gpio_direction_output(fts_data->pdata->reset_gpio, 1);
            } else if (*transition == MSM_DRM_RESET_EARLY_LOW) {
                gpio_direction_output(fts_data->pdata->reset_gpio, 0);
...
#if HTC_FEATURE
    if(!tamper_flag) {
        ts_data->debug_mask = TOUCH_DOWN_UP_LOG;
        pr_info("Debug level = 0x%08X", ts_data->debug_mask);
    } else {
        ts_data->debug_mask = 0;
    }
#endif
#if FTS_POWER_SOURCE_CUST_EN
...
#if defined(CONFIG_DRM_MSM)
    fts_data->drm_notifier.notifier_call = drm_notifier_callback;
    ret = msm_drm_register_client(&fts_data->drm_notifier);
    if (ret < 0) {
        FTS_ERROR("%s: Failed to register drm notifier client: %d\n", __func__, ret);
    }
#elif defined(CONFIG_FB)
...
#if defined(CONFIG_DRM_MSM)
    msm_drm_unregister_client(&ts_data->drm_notifier);
#elif defined(CONFIG_FB)
