...
enum enumTestItem_FT8607 {
    ...
    Code_FT8607_FW_VERSION_TEST = 25,
    Code_FT8607_FACTORY_ID_TEST = 26,
};
...
static int ft8607_FwVersionTest(bool* bTestResult);
static int ft8607_FactoryIdTest(bool* bTestResult);
...
bool start_test_ft8607(void)
{
    ...
    for (item_count = 0; item_count < test_data.test_num; item_count++) {
        ...
        ///////////////////////////////////////////////////////FT8607_FW_VERSION_TEST
        if (Code_FT8607_FW_VERSION_TEST == test_data.test_item[item_count].itemcode)
        {
            ret = ft8607_FwVersionTest(&temp_result);
            if (ERROR_CODE_OK != ret || (!temp_result))
            {
                test_result = false;
                test_data.test_item[item_count].testresult = RESULT_NG;
        ...
        ///////////////////////////////////////////////////////FT8607_FACTORY_ID_TEST
        if (Code_FT8607_FACTORY_ID_TEST == test_data.test_item[item_count].itemcode)
        {
            ret = ft8607_FactoryIdTest(&temp_result);
            if (ERROR_CODE_OK != ret || (!temp_result))
            {
                test_result = false;
                test_data.test_item[item_count].testresult = RESULT_NG;
...
static int ft8607_FwVersionTest(bool* bTestResult)
{
    ...
    {
        sys_delay(60);
        ReCode = read_reg(REG_FWVERSION, &regData);
        ...
        FwVersion = ft8607_basic_thr.FW_VER_VALUE;

        FTS_TEST_DBG("\r\nGet FwVersion:0x%02x, Set FwVersion: 0x%02x  \n", regData, FwVersion);

        FTS_TEST_SAVE_INFO("FwVersion:0x%02x, Set FwVersion: 0x%02x, ", regData, FwVersion);
    ...
    if (ReCode != ERROR_CODE_OK)
    {
        FTS_TEST_ERROR(" EnterFactory failed.. Error Code: %d \n", ReCode);
        btmpresult = false;
        goto TEST_ERR;
    }
...
/************************************************************************
* Name: FT8607_CheckItem_FactoryIdTest
...
static int ft8607_FactoryIdTest(bool* bTestResult)
{
    ...
    sys_delay(60);
    ReCode = read_reg(REG_FACTORYID, &regData);
    if (ReCode != ERROR_CODE_OK)
    {
    ...
    FactoryId = ft8607_basic_thr.Factory_ID_Number;

    FTS_TEST_DBG("\r\nGet FactoryId:0x%02x, Set FactoryId: 0x%02x  \n", regData, FactoryId);

    FTS_TEST_SAVE_INFO("FactoryId:0x%02x, Set FactoryId: 0x%02x, ", regData, FactoryId);
...
static int ft8607_short_test(bool *test_result)
{
    ...
    for (i = 0; i < 1; i++) {
        ret = weakshort_get_adc_data_incell(TEST_RETVAL_00, rx_num, all_adc_data_num * 2, adcdata);
        sys_delay(50);
        if (ERROR_CODE_OK != ret) {
            temp_result = false;
...
void init_basicthreshold_ft8607(char *strIniFile)
{
    ...
    ft8607_basic_thr.FW_VER_VALUE = fts_atoi(str);

    //////////////////////////////////////////////////////////// Factory ID
    GetPrivateProfileString("Basic_Threshold","Factory_ID_Number","255",str,strIniFile);
    ft8607_basic_thr.Factory_ID_Number = fts_atoi(str);
...
void init_testitem_ft8607(char  *strIniFile)
{
    ...
    ft8607_item.FW_VERSION_TEST = fts_atoi(str);

    ///////////////////////////////////FACTORY_ID_TEST
    GetPrivateProfileString("TestItem", "FACTORY_ID_TEST", "0", str, strIniFile);
    ft8607_item.FACTORY_ID_TEST = fts_atoi(str);
...
void set_testitem_sequence_ft8607(void)
{
    ...
    //////////////////////////////////////////////////FW Version Test
    if ( ft8607_item.FW_VERSION_TEST == 1)
    {
        fts_set_testitem(Code_FT8607_FW_VERSION_TEST);
    }

    //////////////////////////////////////////////////FACTORY_ID_TEST
    if ( ft8607_item.FACTORY_ID_TEST == 1)
    {
        fts_set_testitem(Code_FT8607_FACTORY_ID_TEST);
    }
