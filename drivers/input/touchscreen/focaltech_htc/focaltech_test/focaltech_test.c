...
#if HTC_FEATURE
static int fts_read_diag_data(struct fts_test_data *ts_data)
{
    int ret = 0;
    struct fts_test_data *data = ts_data;
    u8 tx_num = 0;
    u8 rx_num = 0;
    int va_num = 0;
    ...
    ret = enter_factory_mode();
    if (ret) {
        FTS_TEST_ERROR("%s: failed to enter fac mode", __func__);
        goto err_state;
    }

    ret = start_scan_incell();
    if (ret) {
        FTS_TEST_ERROR("%s: failed to scan incell info", __func__);
        goto err_state;
    }

    ret = write_reg(FACTORY_REG_LINE_ADDR, 0xAD);
    if (ret) {
        FTS_TEST_ERROR("%s: write 0xAD to reg01 failed", __func__);
        goto err_state;
    }

    // Read mutual-capacity data
    switch(data->diag_command) {
        case CAP_DELTA_DATA:
            ret = write_reg(FACTORY_REG_DATA_SELECT, 0x01);
            if (ret) {
                FTS_TEST_ERROR("%s: write 0x01 to reg06 failed", __func__);
                goto err_state;
            }
            break;
        case CAP_RAW_DATA:
            ...
            if (ret) {
                FTS_TEST_ERROR("%s: write 0x00 to reg06 failed", __func__);
                goto err_state;
            }
            break;
        default:
            FTS_TEST_ERROR("%s: wrong diag command", __func__);
            break;
    }

    ret = read_mass_data(FACTORY_REG_RAWDATA_ADDR, va_num * 2, test_data.buffer);
    if (ret) {
        FTS_TEST_ERROR("%s: Failed to Get RawData of channel.", __func__);
        goto err_state;
    }
...
static ssize_t htc_diag_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t count = 0;
    int ret = 0;
    int row, col;
...
#if 0
    count += snprintf(buf, PAGE_SIZE, "diagcmd: %d\n", test_data.diag_command);
#endif

    ret = fts_read_diag_data(&test_data);
    if (ret) {
        FTS_TEST_ERROR("%s: Failed to fts_read_diag_data", __func__);
    }

    for (row = 0; row < tx_num; row++) {
#if 0
        count += snprintf(buf + count, PAGE_SIZE, "Ch_%02d:  ", row + 1);
#endif
        for (col = 0; col < rx_num; col++) {
            count += snprintf(buf + count, PAGE_SIZE, "%5d, ", test_data.buffer[row * rx_num + col]);
        }
...
static ssize_t htc_diag_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    unsigned int input;
    ...
    if (input == 1)
        test_data.diag_command = CAP_DELTA_DATA;
    else if (input == 2)
        test_data.diag_command = CAP_RAW_DATA;
    else
        return -EINVAL;
...
#if HTC_FEATURE
static DEVICE_ATTR(diag, S_IRUGO | S_IWUSR, htc_diag_show, htc_diag_store);
#endif
...
int fts_test_init(struct i2c_client *client)
{
    ...
    ret = get_channel_num();
    if (ret) {
        FTS_TEST_ERROR("%s: failed to get_channel_num", __func__);
        enter_work_mode();
        return ret;
    }
    FTS_TEST_SAVE_INFO("tx_num:%d, rx_num:%d\n",
    ...
    enter_work_mode(); // goto factory mode in get_channel_num
    /* Allocate test data buffer */
    len = (test_data.screen_param.tx_num + 1) * test_data.screen_param.rx_num;
    test_data.buffer = (int *)fts_malloc(len * sizeof(int));
    if (NULL == test_data.buffer) {
        FTS_TEST_ERROR("test_data.buffer malloc failed");
        return -ENOMEM;
    }
    memset(test_data.buffer, 0, len * sizeof(int));
#endif
