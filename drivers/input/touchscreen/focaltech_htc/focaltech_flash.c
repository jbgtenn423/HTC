 ...
 ***********************************************************************/
int fts_read_file(char *file_name, u8 **file_buf, bool isHtc)
{
    int ret = 0;
    ...
    if (isHtc)
        snprintf(file_path, FILE_NAME_LENGTH, "%s%s", HTC_TP_FW_PATH, file_name);
    else
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_upgrade_bin(struct i2c_client *client, char *fw_name, bool force, bool isHtc)
{
    int ret = 0;
    u32 fw_file_len = 0;
    u8 *fw_file_buf = NULL;
    ...
    ret = fts_read_file(fw_name, &fw_file_buf, isHtc);
    if ((ret < 0) || (ret < FTS_MIN_LEN) || (ret > FTS_MAX_LEN_FILE)) {
        FTS_ERROR("read fw bin file(%s) fail, len:%d", (isHtc)? "/vendor/firmware": "sdcard", ret);
        goto err_bin;
    }
    ...
    if(isHtc) {
        if(!strcmp(fw_name, "focaltech_all.bin")) {
            FTS_INFO("fw_name = %s", fw_name);
            fw_ver = fw_file_buf[0x110E];
        }

        if(!strcmp(fw_name, "focaltech_app.bin")) {
            FTS_INFO("fw_name = %s", fw_name);
            fw_ver = fw_file_buf[0x10E];
        }

        ret = fts_fwupg_get_ver_in_tp(client, &host_tp_ver);
        if (ret < 0) {
            FTS_ERROR("%s: fts_fwupg_get_ver_in_tp failed", __func__);
            goto err_bin;
        }

        if (fw_ver == host_tp_ver) {
            FTS_INFO("no need to upgrade");
            goto err_bin;
        }
    }
