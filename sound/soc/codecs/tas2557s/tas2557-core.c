...
/* HTC_AUD_START */
static void htc_tas2557_status_reg_print(struct tas2557_priv *pTAS2557)
{
	unsigned int nPowerStatus, nIntStatus1, nIntStatus2;

	pTAS2557->read(pTAS2557, channel_left, TAS2557_POWER_UP_FLAG_REG, &nPowerStatus);
	pTAS2557->read(pTAS2557, channel_left, TAS2557_FLAGS_1, &nIntStatus1);
	pTAS2557->read(pTAS2557, channel_left, TAS2557_FLAGS_2, &nIntStatus2);
	dev_err(pTAS2557->dev, "%s L power: 0x%08x, int1: 0x%08x, int2: 0x%08x\n", __func__, nPowerStatus, nIntStatus1, nIntStatus2);
	pTAS2557->read(pTAS2557, channel_right, TAS2557_POWER_UP_FLAG_REG, &nPowerStatus);
	pTAS2557->read(pTAS2557, channel_right, TAS2557_FLAGS_1, &nIntStatus1);
	pTAS2557->read(pTAS2557, channel_right, TAS2557_FLAGS_2, &nIntStatus2);
	dev_err(pTAS2557->dev, "%s R power: 0x%08x, int1: 0x%08x, int2: 0x%08x\n", __func__, nPowerStatus, nIntStatus1, nIntStatus2);
}

static void htc_tas2557_recover(struct tas2557_priv *pTAS2557)
{
	int nResult = 0;

	dev_info(pTAS2557->dev, "%s re-load firmware\n", __func__);
	switch (pTAS2557->mnLPGID) {
	case TAS2557_PG_VERSION_2P1:
		nResult = request_firmware_nowait(THIS_MODULE, 1, TAS2557_PG21_FW_NAME,
		pTAS2557->dev, GFP_KERNEL, pTAS2557, tas2557_fw_ready);
		dev_warn(pTAS2557->dev, "%s loading %s\n", __func__, TAS2557_PG21_FW_NAME);
		break;

	default:
		nResult = request_firmware_nowait(THIS_MODULE, 1, TAS2557_FW_NAME,
		pTAS2557->dev, GFP_KERNEL, pTAS2557, tas2557_fw_ready);
		dev_warn(pTAS2557->dev, "%s loading %s\n", __func__, TAS2557_FW_NAME);
		break;
	}
}
/* HTC_AUD_END */
...
{
/* HTC_AUD_START: print mnErrCode here because i2c errors might be cleared in tas2557_dev_load_data() */
#if 0
	dev_err(pTAS2557->dev, "%s\n", __func__);
#else
	dev_err(pTAS2557->dev, "%s: mnErrCode 0x%x\n", __func__, pTAS2557->mnErrCode);
	htc_tas2557_status_reg_print(pTAS2557);
#endif
/* HTD_ADU_END */
...
/* HTC_AUD_START: remve clear firmware due to not reload back */
	...
	if (pTAS2557->mpFirmware != NULL) {
		...
		htc_tas2557_recover(pTAS2557); // HTC_AUD
	}
#endif
/* HTD_ADU_END */
	pTAS2557->failsafe_retry = 0;
...
prog_coefficient:
	...
	if (bRestorePower) {
		...
		if (nResult < 0)
			goto end;
		if (pProgram && pProgram->mnAppMode == TAS2557_APP_TUNINGMODE) { // HTC_AUD
...
#else
	unsigned char *nBuf1 = (unsigned char *)kmalloc(128*sizeof(unsigned char), GFP_KERNEL);
	unsigned char *nBuf2 = (unsigned char *)kmalloc(128*sizeof(unsigned char), GFP_KERNEL);
#endif
/* HTC_AUD_END */
